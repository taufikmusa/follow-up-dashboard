...
import { 
  ...
  FileSpreadsheet, Terminal, MessageSquare
} from "lucide-react";
import { 
...
function Modal({ c, onClose, messageTemplate }) {
  if (!c) return null;
...
function Dashboard({ data, onReset }) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto animate-in fade-in zoom-in-95 duration-300">
      {selected && <Modal c={selected} onClose={() => setSelected(null)} messageTemplate={messageTemplate} />}
                ...
                return (
                  <tr key={(c.pgcode || c.email || "") + i} onClick={() => setSelected(c)} className={`hover:bg-[#FFFBE5] transition-colors cursor-pointer group even:bg-gray-50`}>
                    ...
                    <td className="px-5 py-3 text-center" onClick={e => e.stopPropagation()}>
                      <WaBtn customer={c} messageTemplate={messageTemplate} />
                    </td>
                  </tr>
                );
              })}
